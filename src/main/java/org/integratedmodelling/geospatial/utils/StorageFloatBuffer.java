...
/**
 * A {@link DataBuffer data buffer} implementation to handle float type gridded data.
 *
 * <p>Following assumptions are currently made on the state object and its locator companion:
 *
 * <ul>
 *   <li>the state represents a grid object
 *   <li>the values contained are {@link Number} objects
 *   <li>the locator object is used as starting {@link Offset} object to ensure the right choice of
 *       index for the time dimension, while the space position is updated from the {@link Grid}
 *       object using row/col
 *   <li>the raster has a single bank (0)
 * </ul>
 *
...
public class StorageFloatBuffer extends DataBuffer {
  ...
  public StorageFloatBuffer(
    ...
    if (!(space instanceof Space) || ((Space) space).getGrid() == null) {
      throw new IllegalArgumentException(
          "Only gridded state objects are accepted for this buffer object.");
    }
  ...
  private void setValue(
      float value, int index, IStorage<?> state, Function<Object, Object> transformation) {
    int y = (int) (index / width);
    int x = index % width;
  ...
  private void setValue(
      float value, int x, int y, IStorage<?> state, Function<Object, Object> transformation) {
    long spaceOffset = grid.getOffset(x, y);
    Offset off = new Offset(scale, new long[] {0, spaceOffset});
