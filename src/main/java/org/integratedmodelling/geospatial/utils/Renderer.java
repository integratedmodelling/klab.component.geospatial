...
//        ColorScheme scheme = Utils.Json.load(res.getInputStream(), ColorScheme.class);
...
//
//  //  /**
//  //   * Render a given state to an image within a specified viewport using settings from
//  // annotations or
//  //   * defaults.
//  //   *
//  //   * @param state
...
//  //   * @param viewport
//  //   */
//  //  public BufferedImage render(Observation state, Scheduler.Event locator, int[] viewport) {
//  //
//  //    if (state.getSpace() == null
//  //        || !(state.getSpace() instanceof Space && ((Space) state.getSpace()).getGrid() != null))
//  // {
//  //      throw new IllegalArgumentException(
//  //          "cannot render a state as a map unless its space is gridded");
//  //    }
//  //
//  //    var grid =
//  //
...
//  //          Geotools.stateToCoverage(state, locator, DataBuffer.TYPE_FLOAT, Float.NaN, false);
//  //
//  //      //
...
//  //
//  //      Viewport vport = new Viewport(viewport[0], viewport.length == 1 ? viewport[0] :
//  // viewport[1]);
//  //      Envelope envelope = space.getEnvelope();
//  //      Projection projection = space.getProjection();
...
//  //              ((Envelope) envelope).getJTSEnvelope(), screenSize);
//  //      GridCoverageRenderer renderer =
//  //          new GridCoverageRenderer(
//  //              ((ProjectionImpl) projection).getCoordinateReferenceSystem(),
//  //              ((Envelope) envelope).getJTSEnvelope(),
//  //              screenSize,
//  //              w2s);
...
//  //      BufferedImage image =
//  //          new BufferedImage(imagesize[0], imagesize[1], BufferedImage.TYPE_INT_ARGB);
...
//  //  }
//
//  public static Pair<RasterSymbolizer, String> getRasterSymbolizer(Observation observation,
//                                                                   Storage.DoubleScanner scanner,
//                                                                   ContextScope scope) {
//
//    var histogram = scanner.shard().getHistogram();
//    if (histogram.isEmpty()) {
//      return Pair.of(null, NO_DATA_MESSAGE);
//    }
//
//    boolean stateIsBoolean = observation.getType() == Artifact.Type.BOOLEAN;
//    var dataKey = scope.getDigitalTwin().getStorageManager().getStorage(observation).getKey();
//    if (Utils.Numbers.equal(histogram.getMin(), histogram.getMax())) {
//
//      // TODO this won't show anything when we have one value AND no-data - which is
//      // wrong.
//      String label = "";
//      double value = histogram.getMin();
//      if (dataKey != null) {
//        label = dataKey.getLabels().get((int) value);
...
//        return Pair.of(null, label);
//      } else {
//        var colorMap =
//            styleBuilder.createColorMap(
//                new String[] {label, label},
//                new double[] {histogram.getMin() - 0.5, histogram.getMax() + 0.5},
//                new Color[] {new Color(0, 0, 255), new Color(0, 0, 255)},
//                ColorMap.TYPE_RAMP);
...
//            : (dataKey.isOrdered() ? ColorMap.TYPE_INTERVALS : ColorMap.TYPE_VALUES);
//
...
////              colors =
////                  palette.getColors(state.getDataKey() == null ? 256 : state.getDataKey().size());
...
////                System.err.println(
////                    "Value " + data.getSecond() + " is not present in colormap, set to GRAY");
////              }
////              svals.add(p);
////            }
////          }
...
////            colormapType =
////                annotation.get("continuous", Boolean.TRUE)
////                    ? ColorMap.TYPE_RAMP
...
////              labels[i] =
////                  state.getObservable().getName()
////                      + " "
////                      + ((Boolean) pair.getFirst()
...
//        if (dataKey.isOrdered()) {
//
//          colormapType = ColorMap.TYPE_VALUES;
//
//          java.util.List<Pair<Integer, String>> valabs = dataKey.getAllValues();
//          colors = new Color[dataKey.size()];
//          values = new double[dataKey.size()];
//          labels = new String[dataKey.size()];
//
//          Color[] jetcolors = jet(1.0f);
//
...
//            int index =
//                (int)
//                    (((double) (i + 1) / (double) state.getDataKey().size())
...
//
//        } else {
//          if (dataKey.size() <= random20.length) {
//            colors = Arrays.copyOf(random20, dataKey.size());
//          } else if (dataKey.size() <= excel.length) {
//            colors = Arrays.copyOf(excel, dataKey.size());
//          } else {
//            // warn and use random shit
//            //                        Logging.INSTANCE.warn("creating random colors to render " +
//            // state.getDataKey().size()
//            //                                + " categories: results won't be optimal");
//            colors = getRandomColors(dataKey.size());
//          }
//        }
//      }
//    }
...
//      if (Double.isNaN(midpoint) || NumericRange.create(summary.getRange()).contains(midpoint)) {
...
//        }
//      }
//    } else if (dataKey != null && colors != null && (values == null || labels == null)) {
//
//      values = new double[dataKey.size()];
//      labels = new String[dataKey.size()];
//      int i = 0;
//      for (Pair<Integer, String> pair :dataKey.getAllValues()) {
...
//    if (shadedRelief != 0.0) {
//      ShadedRelief srl =
//          styleFactory.createShadedRelief(styleBuilder.literalExpression(shadedRelief));
...
//    for (int i = 0; i < colors.length; ++i) colors[i] = new Color(i, i, i);
//    return colors;
//  }
//
...
//
//    double half = colors.length / 2.;
//    for (int i = 0; i <= half; ++i) colors[i] = new Color(255 - (int) (((i) / half) * 255), 0, 0);
//    for (int i = (int) half + 1; i < colors.length; ++i)
...
//
//    for (int i = 0; i < colors.length; ++i) {
//      if (i <= 29) colors[i] = new Color((int) (129.36 - i * 4.36), 0, 255);
//      else if (i <= 86) colors[i] = new Color(0, (int) (-133.54 + i * 4.52), 255);
//      else if (i <= 141) colors[i] = new Color(0, 255, (int) (665.83 - i * 4.72));
//      else if (i <= 199) colors[i] = new Color((int) (-635.26 + i * 4.47), 255, 0);
//      else colors[i] = new Color(255, (int) (1166.81 - i * 4.57), 0);
//    }
//    return colors;
//  }
//
...
//    ImageIO.write(
//        INSTANCE.createImage(INSTANCE.wave(), 20, 256), "png", new java.io.File("legend.png"));
//  }
//}
