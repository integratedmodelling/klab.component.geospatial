...
public class GeodataIO {
  ...
  public InputStream exportHtml(
    ...
    public File exportObservation(File file, Observation observation, Storage.DoubleScanner scanner, String format, ContextScope scope monitor) {

        boolean addStyle = file.getName().endsWith(".zip");
        boolean samefolder = false; // TODO options.get(OPTION_DO_NOT_CREATE_INDIVIDUAL_FOLDERS, Boolean.FALSE);
        boolean doNotZip = false; // TODO options.get(OPTION_DO_NOT_ZIP_MULTIPLE_FILES, Boolean.FALSE)
...
//                        writeQgisStyleCategories(outQml, state, locator);
                    ...
                    coverage = Geotools.stateToCoverage(observation, scanner, DataBuffer.TYPE_FLOAT,
                                                                      Float.NaN, true, null);
                }
    ...
    private void writeQgisStyleCategories(File qmlFile, Observation state, Storage.DoubleScanner scanner, ContextScope scope) throws Exception {

        DataKey dataKey = scope.getDigitalTwin().getStorageManager().getStorage(state).getKey();
        Pair<RasterSymbolizer, String> rasterSymbolizerPair = Renderer.getRasterSymbolizer(state, scanner, scope);
    ...
    private void writeQgisStyleContinuous(File qmlFile, Observation state, Scheduler.Event locator, ContextScope scope) throws Exception {
