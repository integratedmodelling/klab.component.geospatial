...
public class StacParser {
  ...
  public static JSONObject requestMetadata(String collectionUrl, String type) {
    ...
    if (!data.getString("type").equalsIgnoreCase(type)) {
      throw new KlabResourceAccessException(
          "Data at " + collectionUrl + " is not a valid STAC " + type);
    }
  ...
  public static List<SimpleFeature> getFeaturesFromStaticCollection(JSONObject collectionData) {
    ...
    itemHrefs =
        itemHrefs.stream().map(href -> getUrlOfItem(collectionUrl, collectionId, href)).toList();

    return itemHrefs.stream()
        .map(
            i -> {
              try {
  ...
  public static String getUrlOfItem(String collectionUrl, String collectionId, String href) {
    if (href.startsWith("..")) {
      return collectionUrl.replace("/collection.json", "").replace(collectionId, "")
          + href.replace("../", "");
    }
    if (href.startsWith(".")) {
   ...
   */
  public static JSONObject readAssetsFromCollection(StacResource.Collection collection)
      throws KlabResourceAccessException {
